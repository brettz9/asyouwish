// From https://raw.github.com/ochameau/jetpack-subprocess/master/lib/subprocess.js
// Search for "Brett" below to find modifications made for AYW support
...
require("./subprocess_worker_win.js"); // Brett modified to make path relative
require("./subprocess_worker_unix.js"); // Brett modified to make path relative
*/
const URL_PREFIX = module.uri.replace(/index\.js/, ""); // Brett modified this line to use index.js instead of subprocess.js for apparent need of library code
const Spapi = require('../../specialpowersAPI').SpecialPowersAPI; // Brett added this line needed below for AYW
    ...
    function cleanup() {
        ...
        if(active) {
            ...
            setTimeout(function _done() {
                if (options.done) {
                    try {
                        options.done(new Spapi().wrap({ // Brett wrapped for sake of AYW
                            exitCode: exitCode,
                            stdout: output,
                            stderr: error,
                        }));
    ...
    if (options.stdin) {
       createStdinWriter();
        ...
        if(typeof(options.stdin) == 'function') {
            try {
                options.stdin(new Spapi().wrap({ // Brett wrapped for sake of AYW
                    write: function(data) {
                        writeStdin(data);
                    },
                    close: function() {
