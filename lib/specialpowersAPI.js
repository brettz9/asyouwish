// From http://mxr.mozilla.org/mozilla-central/source/testing/specialpowers/content/specialpowersAPI.js?raw=1
// Brett Zamir modified file to add following line (also renaming Comp... to Cmponents),
//    added a try-catch block to avoid dead object errors (see line with "Brett"), commented out Mock* code, 
//    and added final line exporting SpecialPowersAPI as a module
var Cmponents = require('chrome').components;
...
var Ci = Cmponents.interfaces;
var Cc = Cmponents.classes;
var Cu = Cmponents.utils;
...
function getRawCmponents(aWindow) {
    // If we're running in automation that supports enablePrivilege, then we also
    // provided access to the privileged Cmponents.
    try {
        let win = Cu.waiveXrays(aWindow);
        if (typeof win.netscape.security.PrivilegeManager == 'object') Cu.forcePrivilegedCmponentsForScope(aWindow);
    } catch (e) {}
    return Cu.getCmponentsForScope(aWindow);
}
...
function isWrapper(x) {
    try {// Brett added try-catch to try to avoid dead object errors
          return isWrappable(x) && 
            (typeof x.SpecialPowers_wrappedObject !== "undefined");
...
function wrapPrivileged(obj) {
    ...
    // No double wrapping.
    if (isWrapper(obj)) throw "Trying to double-wrap object!";
...
function unwrapPrivileged(x) {
    ...
    if (!isWrappable(x)) return x;

    // If we have a wrappable type, make sure it's wrapped.
    if (!isWrapper(x)) throw "Trying to unwrap a non-wrapped object!";
...
function crawlProtoChain(obj, fn) {
    ...
    if (proto) return crawlProtoChain(proto, fn);
    return undefined;
};
...
SpecialPowersHandler.prototype.doGetPropertyDescriptor = function (name, own) {
    ...
    if (name == "SpecialPowers_wrappedObject") return {
    ...
    function isWrappedNativeXray(o) {
        if (!Cu.isXrayWrapper(o)) return false;
        var proto = Object.getPrototypeOf(o);
        return /XPC_WN/.test(Cu.getClassName(o, /* unwrap = */ true)) || (proto && /XPC_WN/.test(Cu.getClassName(proto, /* unwrap = */ true)));
    }

    // Case 1: Own Properties.
    //
    // This one is easy, thanks to Object.getOwnPropertyDescriptor().
    if (own) desc = callGetOwnPropertyDescriptor(obj, name);
    ...
    else if (!isWrappedNativeXray(this.wrappedObject)) desc = crawlProtoChain(obj, function (o) {
        return callGetOwnPropertyDescriptor(o, name);
    });
    ...
    // tend to live on the prototypes of Xray-wrapped objects.
        ...
        if (!desc) {
            ...
            if (getter || setter) desc = {
                get: getter,
                set: setter,
                configurable: true,
                enumerable: true
            };
            else if (name in obj) desc = {
                value: obj[name],
                writable: false,
                configurable: true,
                enumerable: true
    ...
    if (desc && 'value' in desc && desc.value === undefined) desc.value = obj[name];
    ...
    function wrapIfExists(key) {
        if (key in desc) desc[key] = wrapPrivileged(desc[key]);
    };
    wrapIfExists('value');
    wrapIfExists('get');
    wrapIfExists('set');
...
function doGetOwnPropertyNames(obj, props) {
    ...
    if (props.indexOf(specialAPI) == -1) props.push(specialAPI);

    // Do the normal thing.
    var flt = function (a) {
        return props.indexOf(a) == -1;
    };
    props = props.concat(Object.getOwnPropertyNames(obj).filter(flt));

    // If we've got an Xray wrapper, include the expandos as well.
    if ('wrappedJSObject' in obj) props = props.concat(Object.getOwnPropertyNames(obj.wrappedJSObject)
        .filter(flt));
...
SpecialPowersHandler.prototype.fix = function () {
    return undefined; /* Throws a TypeError. */
};
...
SPConsoleListener.prototype = {
    observe: function (msg) {
        ...
        if (!m.isScriptError && m.message === "SENTINEL") Services.console.unregisterListener(this);
    },
...
function wrapCallbackObject(obj) {
    ...
    var wrapper = {};
    for (var i in obj) {
        if (typeof obj[i] == 'function') wrapper[i] = wrapCallback(obj[i]);
        else wrapper[i] = obj[i];
    }
    return wrapper;
}
...
*/
    loadChromeScript: function (url) {
        ...
        let assert = json = > {
            ...
            var diagnostic = message ? message : ("assertion @ " + stack.filename + ":" + stack.lineNumber);
            if (err) {
                diagnostic +=
                    " - got " + repr(err.actual) +
    ...
    /*
     * In general, any Cmponents object created for unprivileged scopes is
     * neutered (it implements nsIXPCCmponentsBase, but not nsIXPCCmponents).
     * We override this in certain legacy automation configurations (see the
     * implementation of getRawCmponents() above), but don't want to support
     * it in cases where it isn't already required.
     *
     * In scopes with neutered Cmponents, we don't have a natural referent for
     * things like SpecialPowers.Cc. So in those cases, we fall back to the
     * Cmponents object from the SpecialPowers scope. This doesn't quite behave
     ...
     *
     * It probably wouldn't be too much work to just make SpecialPowers.Cmponents
     * unconditionally point to the Cmponents object in the SpecialPowers scope.
     * Try will tell what needs to be fixed up.
     */
    getFullCmponents: function () {
        return typeof this.Cmponents.classes == 'object' ? this.Cmponents : Cmponents;
    },

    /*
     * Convenient shortcuts to the standard Cmponents abbreviations. Note that
     * we don't SpecialPowers-wrap Cmponents.interfaces, because it's available
     * to untrusted content, and wrapping it confuses QI and identity checks.
     */
    get Cc() {
        return wrapPrivileged(this.getFullCmponents()).classes;
    },
    get Ci() {
        return this.Cmponents.interfaces;
    },
    get Cu() {
        return wrapPrivileged(this.getFullCmponents()).utils;
    },
    get Cr() {
        return wrapPrivileged(this.Cmponents).results;
    },

    /*
     * SpecialPowers.getRawCmponents() allows content to get a reference to a
     * naked (and, in certain automation configurations, privileged) Cmponents
     * object for its scope.
     *
     * SpecialPowers.getRawCmponents(window) is defined as the global property
     * window.SpecialPowers.Cmponents for convenience.
     */
    getRawCmponents: getRawCmponents,

    getDOMWindowUtils: function (aWindow) {
        if (aWindow == this.window.get() && this.DOMWindowUtils != null) return this.DOMWindowUtils;
    ...
    _setTimeout: function (callback) {
        ...
        if (typeof window != 'undefined') setTimeout(callback, 0);
        // for mochitest-plain
        else content.window.setTimeout(callback, 0);
    },
    ...
    _delayCallbackTwice: function (callback) {
        function delayedCallback() {
            function delayAgain(aCallback) {
                ...
                if (typeof window != 'undefined') setTimeout(aCallback, 0);
                // For mochitest-plain
                else content.window.setTimeout(aCallback, 0);
            }
    ...
    pushPermissions: function (inPermissions, callback) {
        ...
        for (var p in inPermissions) {
            ...
            if (typeof permission.allow !== 'boolean') {
                ...
                perm = permission.allow ? Ci.nsIPermissionManager.ALLOW_ACTION : Ci.nsIPermissionManager.DENY_ACTION;
            }

            if (permission.remove == true) perm = Ci.nsIPermissionManager.UNKNOWN_ACTION;
            ...
            if (permission.remove == true) todo.op = 'remove';
        ...
        for (var action in inPrefs) { /* set|clear */
            for (var idx in inPrefs[action]) {
                ...
                if (prefs.getPrefType(prefName) != prefs.PREF_INVALID) {
                    ...
                    if ((prefs.prefHasUserValue(prefName) && action == 'clear') || (action == 'set')) originalValue = this._getPref(prefName, prefType);
                } else if (action == 'set') {
                    /* prefName doesn't exist, so 'clear' is pointless */
                    if (aPref.length == 3) {
                        prefType = "COMPLEX";
                    } else if (aPref.length == 2) {
                        if (typeof (prefValue) == "boolean") prefType = "BOOL";
                        else if (typeof (prefValue) == "number") prefType = "INT";
                        else if (typeof (prefValue) == "string") prefType = "CHAR";
                    }
                }

                /* PREF_INVALID: A non existing pref which we are clearing or invalid values for a set */
                if (prefType == prefs.PREF_INVALID) continue;

                /* We are not going to set a pref if the value is the same */
                if (originalValue == prefValue) continue;
  ...
  */
    ...
    autoConfirmAppInstall: function (cb) {
        this.pushPrefEnv({
            set: [
                ['dom.mozApps.auto_confirm_install', true]
            ]
        }, cb);
    },
    ...
    autoConfirmAppUninstall: function (cb) {
        this.pushPrefEnv({
            set: [
                ['dom.mozApps.auto_confirm_uninstall', true]
            ]
        }, cb);
    },
    ...
    addObserver: function (obs, notification, weak) {
        ...
        if (typeof obs == 'object' && obs.observe.name != 'SpecialPowersCallbackWrapper') obs.observe = wrapCallback(obs.observe);
    ...
    _getPref: function (aPrefName, aPrefType, aIid) {
        ...
        if (val == null || val[0] == null) throw "Error getting pref";
        return val[0];
    },
    ...
    _getAutoCompletePopup: function (window) {
        return this._getTopChromeWindow(window).document.getElementById("PopupAutoComplete");
    },
    ...
    getFormFillController: function (window) {
        return Cmponents.classes["@mozilla.org/satchel/form-fill-controller;1"].getService(Cmponents.interfaces.nsIFormFillController);
    },
    ...
    isBackButtonEnabled: function (window) {
        return !this._getTopChromeWindow(window).document.getElementById("Browser:Back")
    ...
    get XPCOMABI() {
        if (this._xpcomabi != null) return this._xpcomabi;

        var xulRuntime = Cc["@mozilla.org/xre/app-info;1"].getService(Cmponents.interfaces.nsIXULAppInfo)
            .QueryInterface(Cmponents.interfaces.nsIXULRuntime);
    ...
    executeSoon: function (aFun, aWin) {
        ...
        var runnable = {};
        if (aWin) runnable = Cu.createObjectIn(aWin);
        runnable.run = aFun;
    ...
    get OS() {
        if (this._os != null) return this._os;
    ...
    addCategoryEntry: function (category, entry, value, persists, replace) {
        Cmponents.classes["@mozilla.org/categorymanager;1"].
        getService(Cmponents.interfaces.nsICategoryManager).
    ...
    copyString: function (str, doc) {
        Cmponents.classes["@mozilla.org/widget/clipboardhelper;1"].
        getService(Cmponents.interfaces.nsIClipboardHelper).
        copyString(str, doc);
    },
    ...
    get focusManager() {
        if (this._fm != null) return this._fm;

        this._fm = Cmponents.classes["@mozilla.org/focus-manager;1"].
        getService(Cmponents.interfaces.nsIFocusManager);
    ...
    getClipboardData: function (flavor, whichClipboard) {
        if (this._cb == null) this._cb = Cmponents.classes["@mozilla.org/widget/clipboard;1"].
        getService(Cmponents.interfaces.nsIClipboard);
        if (whichClipboard === undefined) whichClipboard = this._cb.kGlobalClipboard;

        var xferable = Cmponents.classes["@mozilla.org/widget/transferable;1"].
        createInstance(Cmponents.interfaces.nsITransferable);
        ...
        xferable.init(this._getDocShell(content.window || window)
            .QueryInterface(Cmponents.interfaces.nsILoadContext));
        ...
        return data.QueryInterface(Cmponents.interfaces.nsISupportsString).data;
    },

    clipboardCopyString: function (preExpectedVal, doc) {
        var cbHelperSvc = Cmponents.classes["@mozilla.org/widget/clipboardhelper;1"].
        getService(Cmponents.interfaces.nsIClipboardHelper);
    ...
    supportsSelectionClipboard: function () {
        if (this._cb == null) {
            this._cb = Cmponents.classes["@mozilla.org/widget/clipboard;1"].
            getService(Cmponents.interfaces.nsIClipboard);
        }
    ...
    swapFactoryRegistration: function (cid, contractID, newFactory, oldFactory) {
        ...
        var componentRegistrar = Cmponents.manager.QueryInterface(Cmponents.interfaces.nsIComponentRegistrar);
        ...
        if (cid == null) {
            if (contractID != null) {
                ...
                oldFactory = Cmponents.manager.getClassObject(Cmponents.classes[contractID],
                Cmponents.interfaces.nsIFactory);
            } else {
                return {
    ...
    _getInfoFromPermissionArg: function (arg) {
        ...
        if (typeof (arg) == "string") {
            // It's an URL.
            url = Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService)
                .newURI(arg, null, null)
                .spec;
        } else if (arg.manifestURL) {
            // It's a thing representing an app.
            let appsSvc = Cc["@mozilla.org/AppsService;1"].getService(Ci.nsIAppsService)
            ...
            // It's a document.
            isSystem = (arg.nodePrincipal instanceof Ci.nsIPrincipal) && Cc["@mozilla.org/scriptsecuritymanager;1"].
    ...
    addPermission: function (type, allow, arg) {
        ...
        if (typeof allow !== 'boolean') {
            ...
            permission = allow ? Ci.nsIPermissionManager.ALLOW_ACTION : Ci.nsIPermissionManager.DENY_ACTION;
        }
...
this.getRawCmponents = getRawCmponents;

exports.SpecialPowersAPI = SpecialPowersAPI;
