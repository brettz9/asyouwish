...
exports.main = function () {
    ...
    var _ = require('l10n').get,
        ...
        */
        setByPref = function (pref) {
            var workerURL, allowedWebsitesApproved;
            switch(pref) {
                ...
                case 'allowedWebsitesApproved':
                    ...
                    break;
                case 'addonWebsites':
                    removeWorkers(); // In case user manually changes option, we should get rid of everything active (todo: we could do a diff to avoid removing unchanged items)
                    addWorkers();
                    break;
                default:
        ...
        */
        ...
        /**
        * Remove an "addon" website worker by URL
        * @param {String} website The "addon" website to be removed
        */
        removeWorker = function (website) {
            workers[website] && workers[website].destroy();
            optionsPanel.port.emit('workerRemoved', prefs.addonWebsites[website]);
        },
        /**
        * Remove all "addon" website workers
        */
        ...
        addWorker = function (website) {
            ...
            optionsPanel.port.emit('setAddonWebsites', JSON.parse(prefs.addonWebsites));
        },
        addWorkers = function () {
            var addonURL;
            for (addonURL in prefs.addonWebsites) {
                if (prefs.addonWebsites.hasOwnProperty(addonURL)) {
                    addWorker(addonURL);
                }
            }
        },
    ...
    optionsPanel.port.on('removeAllowedWebsites', function (websitesToRemove) {
        websitesToRemove.forEach(function (websiteToRemove) {
            searchAndSet('allowedWebsites', websiteToRemove, true, function () {
                ...
                removeWorker(websiteToRemove); // In case executing as an addon
            });
            ...
            searchAndSet('addonWebsites', websiteToRemove, true, function () {
    ...
    optionsPanel.port.on('addAddonWebsite', function (website) {
        ...
        try {
            aywPrivManager.sitePrefsManager.newURI(website); // Ensure first it is a valid website but not throwing errors here
            addWorker(website);
            searchAndSet('addonWebsites', website, false);
        }
        catch (e) {
            // Fail silently
        }
            ...
            AsYouWish = { // __exposedProps__ only appears it can work with global object as opposed to global function, etc.
                ...
                */
                register: function (config) {
                    // meta-data "license", "developer" (name, url), "version", "name", "description"
