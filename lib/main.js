...
1) Utilize meta-data "license", "developer" (name, url), "version", "name", "description" in UI

2) Sort privilege whitelist
2) Make way for privilege whitelist to only allow specific namespaces
2) i18n-ize short priv names too
2) Settle on full sdk paths if required (but avoid "sdk/" prefix for consumers?)

3) Dead object errors

4) Include full-featured (all AsYouWish methods) "addon" demo
4) Readme todos (after "addon" release) & Update release notes
    ...
    var _ = require('l10n').get,
        ...
        */
        recursiveRegistrationCheck = function (userConfig, prefCfg) {
            ...
            if (userConfig && !prefCfg) {console.log('a');
                return false;
            }
