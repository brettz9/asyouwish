/*globals exports, require, Cc, Ci, Cu, XPCNativeWrapper, Proxy */
/*
...
exports.main = function () {
    ...
    var _ = require('l10n').get,
        ...
        modulePermittedPropertiesMap = {},
        modulePermittedMethodsMap = {},
        /**
        ...
        */
        getPrivilegedObject = function getPrivilegedObject (privType) {
            ...
            switch (privType) {
                ...
                case 'indexed-db': obj = require('indexed-db'); break;
                ...
                case 'system': obj = require('system'); break;
                ...
                case 'x-subprocess': obj = require('./subprocess/lib/index.js'); break;
        ...
        */
        log = function log (msg) {
            ...
            consoleService.logStringMessage('AsYouWish: ' + msg);
            // console.log('AsYouWish: ' + msg); // not working
        },
        /**
        ...
        privRetriever = function privRetriever (setPrefs, currentRequestIndex) {
            ...
            */
            return function (requestedPriv) {
                function _wrapMethod (o, f) { // e.g.,  xhr.XMLHttpRequest and xhr
                    function construct(constructor, args, o) {
                        /*
                        // Has apparent problems with instanceof (also note that before passed in constructor was a function, not a method name)
                        function F() {
                        ...
                        switch (args.length) {
                            ...
                            case 1:
                                return new o[constructor](args[0]);
                            case 2:
                                return new o[constructor](args[0], args[1]);
                            case 3:
                                return new o[constructor](args[0], args[1], args[2]);
                            case 4:
                                return new o[constructor](args[0], args[1], args[2], args[3]);
                            case 5:
                                return new o[constructor](args[0], args[1], args[2], args[3], args[4]);
                            case 6:
                                return new o[constructor](args[0], args[1], args[2], args[3], args[4], args[5]);
                            default:
                                throw 'Please report to AsYouWish developer that not enough arguments are supported';
                        }
                    }
log(f);
                    var proxy = new Proxy(o[f], {
                        get: function (target, name) {
log('abc');
                            var that = this;
                            return function () {
                                return that.construct(target, arguments);
                            };
                        },
                        construct: function (target, args) {
log('args:'+args);
                            var p, tempObj = {},
                                obj = construct(f, args, o);
                            for (p in obj) {
                                if (typeof obj === 'function') {
                                    tempObj[p] = 'r';
                                }
                                else { // Todo restrict access based on whether genuinely read-write
                                    tempObj[p] = 'wr';
                                }
                            }
                            obj.__exposedProps__ = tempObj;
log(JSON.stringify(tempObj));
                    ...
                    proxy.__exposedProps__ = tempObj;
//log(JSON.stringify(tempObj));
                    return proxy;
                }
                    ...
                    proxy = new Proxy(
                        ...
                        {/*
                            has: function (target, name) {
                                ...
                                return modulePermittedPropertiesMap[requestedPriv].indexOf(name) > -1 && requiredObjWithPrivs[name] !== undefined;
                            },
                            deleteProperty: function (target, name) {
                                if (target.hasOwnProperty(name)) {
                                    delete target[name];
                                }
                                else if (modulePermittedPropertiesMap[requestedPriv].indexOf(name) > -1) {
                                    delete requiredObjWithPrivs[name];
                                }
                            },
                            set: function(target, name, val) {
                                if (target.hasOwnProperty(name)) {
                                    ...
                                    requiredObjWithPrivs[name] = val;
                                }
                            },*/
                            get: function(target, name) { // This only works if property has been added to __exposedProps__
log(name + (typeof requiredObjWithPrivs[name]));
                                return target.hasOwnProperty(name) ?
                                    target[name] :
                                    ((
                                        modulePermittedMethodsMap[requestedPriv].indexOf(name) > -1 &&
                                        !name.match(/^[A-Z]/)
                                    ) ?
                                        function () {
log('func');
                                            return requiredObjWithPrivs[name].apply(requiredObjWithPrivs, arguments);
                                        } :
                                        (typeof requiredObjWithPrivs[name] === 'function' ? _wrapMethod(requiredObjWithPrivs, name) : requiredObjWithPrivs[name]));
                            }
                        }
                    );

                    // We aren't allowed to dynamically add to __exposedProps__ once on an object, so we make a temporary object and assign it as a whole to be the __exposedProps__
                    tempObj = {
                        // __noSuchMethod__: 'wr' // Didn't work as function of same name (just worked as proxy property)
                    };
                    modulePermittedMethodsMap[requestedPriv].forEach(function (method) {
                        tempObj[method] = 'r';
                    });
                    modulePermittedPropertiesMap[requestedPriv].forEach(function (method) {
                        tempObj[method] = 'wr'; // Todo: change our setup to try writing to the property and if error, use a new map object which will write just 'r' here
                    });

                    //throw requiredObjWithPrivs.XMLHttpRequest;
//throw JSON.stringify(tempObj); //{"getRequestCount":"r","XMLHttpRequest":"r"}
                    proxy.__exposedProps__ = tempObj; // We can also add directly onto proxy if exposed here
log(JSON.stringify(tempObj));


                    return approvedObjects.push(proxy);
    ...
    ['console', 'chrome', 'addon-page', 'l10n', 'self', 'simple-prefs', 'timers', 'base64', 'clipboard', 'context-menu', 'hotkeys', 'indexed-db', 'notifications', 'page-mod', 'page-worker', 'panel', 'passwords', 'private-browsing', 'querystring', 'request', 'selection', 'simple-storage', 'system', 'tabs', 'url', 'widget', 'windows', 'toolkit/loader', 'sdk/console/plain-text', 'sdk/console/traceback', 'sdk/content/content', 'sdk/content/loader', 'sdk/content/symbiont', 'sdk/content/worker', 'sdk/core/heritage', 'sdk/core/namespace', 'sdk/core/promise', 'sdk/event/core', 'sdk/event/target', 'sdk/frame/hidden-frame', 'sdk/frame/utils', 'sdk/io/byte-streams', 'sdk/io/file', 'sdk/io/text-streams', 'sdk/loader/cuddlefish', 'sdk/loader/sandbox', 'sdk/net/url', 'sdk/net/xhr', 'sdk/page-mod/match-pattern', 'sdk/platform/xpcom', 'sdk/preferences/service', 'sdk/system/environment', 'sdk/system/events', 'sdk/system/runtime', 'sdk/system/unload', 'sdk/system/xul-app', 'sdk/tabs/utils', 'sdk/test/assert', 'sdk/test/harness', 'sdk/test/httpd', 'sdk/test/runner', 'sdk/util/collection', 'sdk/util/deprecate', 'sdk/util/list', 'sdk/util/uuid', 'sdk/window/utils', 'x-subprocess'].forEach(function (module) {
        var p, privObj = getPrivilegedObject(module);
        modulePermittedMethodsMap[module] = [];
        modulePermittedPropertiesMap[module] = [];
        for (p in privObj) {
            if (typeof privObj[p] === 'function') {
                modulePermittedMethodsMap[module].push(p);
            }
            else {
                modulePermittedPropertiesMap[module].push(p);
            }
        }
    });
    ...
    requestPrivs_Inject = new ContentDocumentGlobalCreatedObserverBuilder({
        ...
        */
        observed: function observed (subject) { // subject is nsIDOMWindow
            var unsafeWindow, mainWindow, AsYouWish,
            ...
            AsYouWish = { // __exposedProps__ only appears it can work with global object as opposed to global function, etc.
                requestPrivs: function (requestedPrivs, callback, errCallback) {
                    try {
                        var currReqIdx, isSync, errBack = wrapErrorFunc(errCallback),
                            /**
                            ...
                            */
                            cb = function () {
                                ...
                                var args = [].slice.call(arguments); // Wrapping each arg in __noSuchMethod__ didn't work here either
                        ...
                        */
                        ...
                        return requestPrivsExport(false, currReqIdx, null, function () {
                            notify(currReqIdx);
                        });
                    }
                    catch (e) {
