...
3) Readme todos (after "addon" release) & Update release notes
    ...
    f) A preferences 'add-on' could list and edit these "addons" or add within normal add-ons dialog.
        ...
        setByPref = function (pref) {
            ...
            switch(pref) {
                ...
                case 'addons':
                    ...
                    // register/unregister/isRegistered to work with this
                    // addAddonWebsite, removeAddonWebsites
                    for (workerURL in workers) {
                        if (workers.hasOwnProperty(workerURL)) {
                            workers[workerURL].destroy();
                        }
                    }
                    optionsPanel.port.emit('workersRemoved');
                    for (addonURL in prefs.addons) {
                        if (prefs.addons.hasOwnProperty(addonURL)) {
                            workers[addonURL] = pageWorker.Page({
                                contentURL: addonURL
                            });
                            optionsPanel.port.emit('workerAdded', prefs.addons[addonURL]);
                        }
                    }
                    break;
                default:
