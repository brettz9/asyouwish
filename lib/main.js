...
1) Avoid problem with gbrowser in page-worker for restartless "addons" requesting privileges?
2) Ability to call unregister/reregister from website, e.g,. to install addon without restart (ensure same code is ok)?
...
3) Allow invoking of a specific URL at start-up optionally within a hidden DOM window so no visible tabs would need to
    ...
    a) Create pseudo-addon loader with page-worker (allow meta-attributes in "custom" property to be displayed along with it)
        i) Utilize meta-data "license", "author", "version", "name", "fullName", "id", "description"
4) Include full-featured (all AsYouWish methods) "addon" demo
5) Document way to allow genuine registration of add-ons without XPI?

6) Readme todos (after "addon" release) & Update release notes
    ...
    e) Find reference to Comparison to Addons to rmention support has now been added for pseudo-addons (and remove mention of planned addons, including in version.txt file and README future todos)

    f) After new dialog (with addon section) so dialog is up to date and familiar: Show screenshots of dialog, etc. as part of pages your users must read--pointing them here might save you the trouble, and ensure that it is hosted at a site which should be secure (and where my wiki has not been made public).
*/
...
exports.main = function () {
        ...
        */
        requestPrivsExport = function requestPrivsExport (setPrefs, currentRequestIndex, successCb, failureCb) {
            ...
            if (!unapprovedPrivs.length && approvedPrivs.length) { // All found (and approved have not been deleted), so call/return with array of objects
                ...
                searchAndSet('allowedWebsitesApproved', request.site, false);
    ...
    optionsPanel.port.on('addAllowedProtocol', function (protocol) {
        ...
        if (protocol.match(/^[a-z][a-z\d+.\-]*$/i)) { // underscore not permitted
            searchAndSet('allowedProtocols', protocol, false);
        }
    });
    ...
    optionsPanel.port.on('addAllowedWebsite', function (website) {
        ...
        try {
            ...
            searchAndSet('allowedWebsites', website, false);
        }
        catch (e) {
            // Fail silently
        }
    ...
    optionsPanel.port.on('removeAllowedWebsites', function (websitesToRemove) {
        websitesToRemove.forEach(function (websiteToRemove) {
            ...
            searchAndSet('allowedWebsitesApproved', websiteToRemove, true);
        });
    });
    ...
    */
    ...
    pageWorker.Page({
        contentScript: '',
        //contentURL: 'D:\wamp\www\ayw\demos\requestPrivs-addon-demo.html'
        contentURL: 'http://127.0.0.1/ayw/demos/requestPrivs-addon-demo.html'
    });
    */
};
