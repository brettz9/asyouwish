...
2) Readme todos (after "addon" release) & Update release notes
    0) Simplify summary at https://addons.mozilla.org/en-US/firefox/addon/as-you-wish/
    ## **How can my "addon" website listen for an unload event to perform clean-up work**
    I haven't seen any way to execute a genuine "unload" event (implemented as a page-worker's contentURL), so currently, you have to listen for a (custom) "x-unload" event (on the window).
    ...
    i) After new dialog (with addon section) so dialog is up to date and familiar: Show screenshots of dialog, etc. as part of pages your users must read--pointing them here might save you the trouble, and ensure that it is hosted at a site which should be secure (and where my wiki has not been made public).
