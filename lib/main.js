...
Remaining "Addon" release todos:

1) i18n content script/fix properties bug?

2) Utilize meta-data "license", "developer" (name, url), "version", "name", "description" in UI
2) Allow isRegistered to check version/properties?

3) Sort privilege whitelist and 
3) Make way for privilege whitelist to only allow specific namespaces
3) i18n-ize short priv names too
3) Settle on full sdk paths if required (but avoid "sdk/" prefix for consumers?)

4) Dead object errors

5) Include full-featured (all AsYouWish methods) "addon" demo
5) Readme todos (after "addon" release) & Update release notes
    0) Simplify summary at https://addons.mozilla.org/en-US/firefox/addon/as-you-wish/
    0) Reconcile wiki/FAQ
    ...
    i) document x-unload event

    j) After new dialog (with addon section) so dialog is up to date and familiar: Show screenshots of dialog, etc. as part of pages your users must read--pointing them here might save you the trouble, and ensure that it is hosted at a site which should be secure (and where my wiki has not been made public).
...
exports.main = function () {
    ...
    var _ = require('l10n').get,
        ...
        */
        ...
        notifyBeforeRegisterAddon = function (currentRequestIndex, mainWindow) {
            ...
            */
            ...
            try {
                ...
                console.log('Problem adding notification dialog (two simultaneous requests?): ' + e);
                require('timers').setTimeout(function () { // Try again
                    notifyBeforeRegisterAddon(currentRequestIndex, mainWindow);
                }, 3000);
            }
        };
    ...
    optionsPanel.port.on('_', function (args) { // Localization helper for content script
        // The replacement below is to fix an apparent bug with properties files:
        // https://bugzilla.mozilla.org/show_bug.cgi?id=825803
        // https://developer.mozilla.org/en-US/docs/XUL/Tutorial/Property_Files#Escape_non-ASCII_Characters
        // optionsPanel.port.emit('_', [args[0], _.apply(null, args).replace(/\\u0020/g, ' ')]);
        optionsPanel.port.emit('_', [args[0], _.apply(null, args)]);
    });
...
console.log('approvedToRemove: '+websitesToRemove);
        websitesToRemove.forEach(function (websiteToRemove) {
