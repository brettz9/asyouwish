...
Todos:
0) Change from notification hanger to a per-window modal to avoid potential confusion on originating sites (and avoid accidental closings of doorhangers); also allows better control of styling of dialog?
0) For FF21 alpha, fix Panel inside widget (see "addon" demos); now showing as empty
1) Dead object errors/memory
2) Namespaced storage
3) nsIProcess and demo? https://github.com/ochameau/jetpack-subprocess/issues/5#issuecomment-9783672
    a) maintain environmental variables to resubmit to each process call
    b) parse cd/chdir to update workdir
    c) need to allow for other arguments?
