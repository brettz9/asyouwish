                ...
                if (approvedObjects.indexOf(requiredObjWithPrivs) === -1) { // Condition probably not necessary
                    ...
                    proxy = new Proxy(
                        ...
                        {get: function(target, name) { // This only works if property has been added to __exposedProps__
                            return name.hasOwnProperty(target) ?
                                target[name] :
                                modulePermittedMethodsMap[requestedPriv].indexOf(name) > -1 ?
                                    function () {
                                        return requiredObjWithPrivs[name].apply(null, arguments);
                                    } :
                                    function () {
                                        return requiredObjWithPrivs[name];
                                    };
                        }}
                    );
...
//throw 'b::'+proxy.__exposedProps__.get;
