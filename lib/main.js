...
"Addon" release todos:
0) License/CRUD request API (and options to allow default selected licenses and CRUD+subscribe/notify choices); browser and server-side API
0) Readme todos:
    a) Show screenshots of dialog, etc. as part of pages your users must read--pointing them here might save you the trouble, and ensure that it is hosted at a site which should be secure (and where my wiki has not been made public).

0) Avoid problem with gbrowser in page-worker for restartless "addons" requesting privileges?
...
2) Allow invoking of a specific URL at start-up optionally within a hidden DOM window so no visible tabs would need to
...
3) Include full-featured (all AsYouWish methods) "addon" demo
4) Document way to allow genuine registration of add-ons without XPI?
