...
1) Add allow messages and requires for other require modules (at least 
    built-in (prioritize file), but also nsIProcess, shared IndexedDB, etc.?)
...
4) Allow blacklists
5) Locales (not supporting yet to continue taking advantage of convenient online testing (which doesn't support locales) without need of local environment)
...
Todos:
0) integration with requirejs as plugin or even include make API the same (and handle non-plugin requiring) so additional script not needed?
...
exports.main = function () {
    ...
    var _ = // require('l10n').get,
        /**
        * Temporary localization wrapper
        * @param {String} msg Message to localize
        * @param {*} arg1,... Additional arguments
        */
        function (msg) {
            return msg + [].slice.call(arguments, 1).join('');
        },
        ...
        * 2) the scheme is https or approved
        ...
        */
        siteAllowed = function siteAllowed (site) {
            ...
            return (
                    (prefs.allowAllProtocols || scheme === 'https' || inJSONArray(prefs.allowedProtocols, scheme)) && 
        ...
        * @returns {Object} The requested object wrapped with __exposedProps__ for security
        */
        getPrivilegedObject = function getPrivilegedObject (privType) {
            ...
            switch (path) {
                case 'api-utils/xhr':
                    obj = require('api-utils/xhr');
                    break;
                case 'api-utils/url':
                    obj = require('api-utils/url');
                    break;
                case 'api-utils/file':
                    obj = require('api-utils/file');
                    break;
        ...
        notify = function $notify (currentRequestIndex, mainWindow) {
            ...
            */
            privMsg = unapprovedPrivs.reduce(function (prev, priv) {
                    return prev + aywPrivManager.getPrivMessage(priv) + '; '; // This is text, not HTML so can't use line breaks
                }, '');
            ...
            notification = noticeObj.show(
                ...
                'requestPrivs',
                _("This website is asking your permission for the POTENTIALLY VERY DANGEROUS ability to access: ", privMsg),
                ...
                [
                    {
                        label: _("I fully trust this website with the ability to access: ", privMsg),
                        accessKey: _("T"),
                        callback: function () {
